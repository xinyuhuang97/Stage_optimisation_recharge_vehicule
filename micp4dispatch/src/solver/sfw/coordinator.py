...
def dispatch(dispatch_config: DispatchConfig, nb_it = 100):
    ...
    for k in range(nb_it):
        ...
        for i in range(1):
            ...
            for n, ev in enumerate(dispatch_config.evses):
                if rd.random() < omega :
                    solution_dict[i][n] = optimize_sub(ev, dispatch_config, slope_fcr_up, slope_fcr_down)
                else :
                    solution_dict[i][n] = copy.deepcopy(last_solution[n])
                curr_solution[n] = {}
                curr_solution[n]["individual penalties"] = lance_curve_penalty(ev, solution_dict[i][n])
            ...
            objectif_value.append((sum((alpha_up * (sum(solution_dict[i][n] ["fcr_up"][t] for n in N) - demand_up[t]) ** 2
                ...
                + (sum(curr_solution[n]["individual penalties"] for n in N)
