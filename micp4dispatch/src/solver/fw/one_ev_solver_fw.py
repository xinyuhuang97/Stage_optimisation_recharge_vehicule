...
def optimize_sub(evse: EVSE, dispatch_config: DispatchConfig, fcr_up_price: list, fcr_down_price: list):
    ...
    for trajectory in 'baseline', 'fcr_up', 'fcr_down':
        ...
        for sense in 'charge', 'discharge':
            activation[trajectory][sense] = m.continuous_var_list(T, name = f"activation_{trajectory}_{sense}")
    ...
    #print(f" sub pb fcr up = {[solution[fcr_up[t]] for t in range(T)]}")
    #print(f" sub pb fcr down = {[solution[fcr_down[t]] for t in range(T)]}")
