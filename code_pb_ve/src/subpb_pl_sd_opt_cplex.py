...
def resolution_subpb_st_opt(data, lambda_k, i, x_bar_k, diff, verbose=False,):
    ...
    # ajoute
    problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=["c_bl_"+str(t)],val=[1]) for t in range(1,T+1)], senses=["G"]*(T), rhs=[x_bar_k["c_bl"][i][t] for t in range(T)])
    problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=["d_bl_"+str(t)],val=[1]) for t in range(1,T+1)], senses=["G"]*(T), rhs=[x_bar_k["d_bl"][i][t] for t in range(T)])
    problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=["s_bl_"+str(t)],val=[1]) for t in range(1,T+1)], senses=["G"]*(T), rhs=[x_bar_k["s_bl"][i][t] for t in range(T)])
    #problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=["c_up_"+str(t),"d_up_"+str(t)],val=[1,-1]) for t in range(1,T+1)], senses=["E"]*(T), rhs=[diff[t] for t in range(T)])
