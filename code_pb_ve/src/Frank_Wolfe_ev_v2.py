...
class Frank_Wolfe_Stochastic:
    def __init__(self, my_instance, nb_ev=0):
        self.my_instance = my_instance    
        if nb_ev!=0:
            self.data = instance_fix_nb(my_instance, nb_ev)
        else:
    ...
    def FW_solve(self, actual_time,  verbose=False, analyse=False, K=100, nk=50):
        ...
        # Boucle principale
        #print("iteration 0")
        #ƒx_bar_0_score = objective_function(data, x_bar_k, s_i_t_min, s_i_max)
        #print(x_bar_0_score)
        ...
        for k in range(K):
            ...
            print("xk",x_k)
            """
            #aaa=np.sum(x_k["c_bl"] - x_k["d_bl"] - x_k["c_up"] + x_k["d_up"],axis=0)
            #print("before",aaa)
            #print(aaa/N)
            """
            print(np.sum(x_k["c_bl"] - x_k["d_bl"],axis=0)/N)
            print(x_k["s_bl"])
            print("============")
            """
            ...
            if k==K-1:
                c_bl_rx=np.zeros((self.N,self.T))
                c_up_rx=np.zeros((self.N,self.T))
                d_bl_rx=np.zeros((self.N,self.T))
                d_up_rx=np.zeros((self.N,self.T))
                s_bl_rx=np.zeros((self.N,self.T))
                s_up_rx=np.zeros((self.N,self.T))
                for i in range(self.N):
                    c_bl_i_rx, d_bl_i_rx, c_up_i_rx, d_up_i_rx , _, _, _, _, s_bl_i_rx, s_up_i_rx, y = resolution_subpb_rx(data, lambda_k,0,x_bar_k, verbose=verbose)
                    c_bl_rx[i]=c_bl_i_rx
                    c_up_rx[i]=c_up_i_rx
                    d_bl_rx[i]=d_bl_i_rx
                    d_up_rx[i]=d_up_i_rx
                    s_bl_rx[i]=s_bl_i_rx
                    s_up_rx[i]=s_up_i_rx
                x_k_rx = {"c_bl": c_bl_rx, "d_bl": d_bl_rx, \
                        "c_up": c_up_rx, "d_up": d_up_rx, "s_bl": s_bl_rx, "s_up": s_up_rx}
                gap_rx = (np.dot(lambda_k,np.sum(x_bar_k["c_bl"] - x_bar_k["d_bl"] - x_bar_k["c_up"] + x_bar_k["d_up"],axis=0)/self.N\
                                    -np.sum(x_k_rx ["c_bl"] - x_k_rx ["d_bl"] - x_k_rx ["c_up"] + x_k_rx ["d_up"],axis=0)/self.N))\
                                    +self.function_h(x_bar_k) - self.function_h(x_k_rx)
            if verbose:
        ...
        print("Gap relaxation : ",gap_rx)
        print("Gap primal dual : ",gap_primal_dual)
        print(self.objective_function(x_k_rx),self.objective_function(x_k),self.objective_function(x_bar_k))
