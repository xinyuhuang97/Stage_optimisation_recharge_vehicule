...
class Frank_Wolfe_Standard_v3:
    ...
    def solve(self, actual_time, verbose=False, analyse=False, K=100, nk=50, n_pre=20, gap_calculate=False, optimize=True, nb_spb=1, active_criterion = "iterations", max_time=3600):
        ...
        for k in range(n_pre):
            ...
            if analyse:
                if k==0:
                    df = pd.DataFrame({"k": [k], "best_score":[min(best_sample_score,x_bar_k_score,x_k_score)], "c_bl":[x_bar_k["c_bl"]],"d_bl":[x_bar_k["d_bl"]],"c_up":[x_bar_k["c_up"]],"d_up":[x_bar_k["d_up"]],"s_bl":[x_bar_k["s_bl"]],"s_up":[x_bar_k["s_up"]],"gap_primal_dual":[gap_primal_dual]})
                else:
                    df = df.append({"k": k, "best_score":min(best_sample_score,x_bar_k_score,x_k_score), "c_bl":x_bar_k["c_bl"],"d_bl":x_bar_k["d_bl"],"c_up":x_bar_k["c_up"],"d_up":x_bar_k["d_up"],"s_bl":x_bar_k["s_bl"],"s_up":x_bar_k["s_up"], "gap_primal_dual":gap_primal_dual}, ignore_index=True)
        update_solution=True
        ...
        #for k in range(n_pre, K):
            ...
            x_k = {"c_bl": c_bl, "d_bl": d_bl, "c_up": c_up, "d_up": d_up, "u_bl": u_bl, "v_bl": v_bl, "u_up": u_up, "v_up": v_up, "s_bl": s_bl, "s_up": s_up}
            #start = time.time()
            delta_k = delta_optimize(x_k, x_bar_k, self.data, actual_time)
            #delta_k = 2/(100+k+2)
            ...
            if analyse:
                df = df.append({"time_elapsed": time_elapsed,"k": k, "best_score":score, "c_bl":x_bar_k["c_bl"],"d_bl":x_bar_k["d_bl"],"c_up":x_bar_k["c_up"],"d_up":x_bar_k["d_up"],"s_bl":x_bar_k["s_bl"],"s_up":x_bar_k["s_up"], "gap_primal_dual":gap_primal_dual}, ignore_index=True)
    ...
    def save_data(self, df, n_pre, n_iteration,optimize):
        ...
        if optimize ==False:
            filename="../result/pb_original/" + "/"+instance_name+"/" + "CFW2_" + str(n_pre) + "_" + str(n_iteration) + "_"  + time.strftime("%Y%m%d-%H%M%S") + ".json"
        else:
            filename="../result/pb_original&booster/" + "/"+instance_name+"/" + "CFW2_" + str(n_pre) + "_" + str(n_iteration) + "_"  + time.strftime("%Y%m%d-%H%M%S") + ".json"
