...
class Frank_Wolfe_Standard_v2:
    ...
    def FW_st_solve(self, actual_time, verbose=False, analyse=False, K=100, nk=50, n_pre=20, gap_calculate=False):
        """!!! gap primal-duale"""
        data=self.data
        # Initialisation
        df=0
        ...
        for k in range(n_pre, K):
            ...
            # Resolution des sous-problemes
            if k%10==0:
                born_inf_i_t=np.zeros((self.N,self.T))
                born_sup_i_t=np.zeros((self.N,self.T))
                y_t_inf=np.zeros((self.T))
                y_t_sup=np.zeros((self.T))
                #c_new_up=np.zeros((self.N,self.T))
                #d_new_up=np.zeros((self.N,self.T))
                diff_i_t=np.zeros((self.N,self.T))
                for i in range(self.N):
                    for t in range(self.T):
                        born_inf_i_t[i][t]=max((self.s_i_t_min[i][t]-x_bar_k["s_bl"][i][t])/self.time_mesh_to_hour,x_bar_k["u_up"][i][t]*self.data["evses"][i]["p_charge_min"]-x_bar_k["v_up"][i][t]*self.data["evses"][i]["p_discharge_max"])
                        born_sup_i_t[i][t]=min((self.s_i_max[i]-x_bar_k["s_bl"][i][t])/self.time_mesh_to_hour,x_bar_k["u_up"][i][t]*self.data["evses"][i]["p_charge_max"]-x_bar_k["v_up"][i][t]*self.data["evses"][i]["p_discharge_min"])
                for t in range(self.T):
                    y_t_inf[t]=np.sum(x_bar_k["c_bl"][:,t]-x_bar_k["d_bl"][:,t])-np.sum(born_sup_i_t[:,t])
                    y_t_sup[t]=np.sum(x_bar_k["c_bl"][:,t]-x_bar_k["d_bl"][:,t])-np.sum(born_inf_i_t[:,t])
                    y_t_up=self.data["announced_capacity"]["up"][actual_time+t]
                    if y_t_inf[t]<y_t_up and y_t_sup[t]<y_t_up:
                        beta=(y_t_up-y_t_inf[t])/(y_t_sup[t]-y_t_inf[t])
                        diff_i_t[:,t]=(1-beta)*born_inf_i_t[:,t]+beta*born_sup_i_t[:,t]
                        #d_new_up[:,t]=beta*x_bar_k["d_bl"][:,t]+(1-beta)*born_sup_i_t[:,t]
                    elif y_t_inf[t]>y_t_up :
                        diff_i_t[:,t]=born_inf_i_t[:,t]
                    else:
                        diff_i_t[:,t]=born_sup_i_t[:,t]
                for i in range(self.N):
                    c_bl_i, d_bl_i, c_up_i, d_up_i, u_bl_i, v_bl_i, u_up_i, v_up_i ,s_bl_i, s_up_i, y = resolution_subpb_st_opt(data, lambda_k,i,x_bar_k, diff_i_t[i], verbose=verbose)
                    c_bl[i]=(c_bl_i)
                    c_up[i]=(c_up_i)
                    d_bl[i]=(d_bl_i)
                    d_up[i]=(d_up_i)
