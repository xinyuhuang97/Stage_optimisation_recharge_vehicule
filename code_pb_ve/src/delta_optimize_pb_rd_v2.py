...
def delta_optimize_pb_rd_v2(x_k, x_bar_k, data ,actual_time,verbose=False):
    N=len(data["evses"])
    ...
    g_bar_i_k_1 = [x_bar_k["c_bl"][i] - x_bar_k["d_bl"][i] + x_bar_k["z_1"][i]  for i in range(N)]
    g_bar_i_k_2 = [x_bar_k["c_bl"][i] - x_bar_k["d_bl"][i] - x_bar_k["z_2"][i]  for i in range(N)]
    
   
    g_i_k_1 = [x_k["c_bl"][i] - x_k["d_bl"][i]  + x_bar_k["z_1"][i] for i in range(N)]
    g_i_k_2 = [x_k["c_bl"][i] - x_k["d_bl"][i] - x_bar_k["z_2"][i] for i in range(N)]
    ...
    for t in range(1,T+1):
        ...
        problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=["y_1_"+str(t)]+["delta_"+str(i) for i in range(0,N)], \
                                                                  val=[N]+[-g_i_k_1[i][t-1]+g_bar_i_k_1[i][t-1] for i in range(0,N) ])], senses=["E"], rhs=[np.sum([g_bar_i_k_1[i][t-1] for i in range(N)])])
        problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=["y_2_"+str(t)]+["delta_"+str(i) for i in range(0,N)], \
                                                                    val=[N]+[-g_i_k_2[i][t-1]+g_bar_i_k_2[i][t-1] for i in range(0,N) ])], senses=["E"], rhs=[np.sum([g_bar_i_k_2[i][t-1] for i in range(N)])])
