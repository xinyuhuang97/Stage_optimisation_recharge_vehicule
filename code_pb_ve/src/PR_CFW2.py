...
from delta_optimize_pb_rd import *
...
class Frank_Wolfe_Reduced_standard_v2:
    def __init__(self,my_instance):
        self.instance=my_instance
        self.algo_name="PR_CFW2"
    ...
    def solve(self, actual_time, verbose=False, analyse=False, K=100, nk=50, n_pre=20, gap_calculate=False, optimize=True, active_criterion="iterations", max_time=3600):
        ...
        for k in range(n_pre):
            ...
            print("val",self.objective_function(x_bar_k))
            if verbose:
                print("best_sample_score",best_sample_score)
                print("x_bar_k_score",x_bar_k_score)
                print("x_k_score",x_k_score)
            ...
            if analyse:
                if k==0:
                    ...
                    df = df.append({"k": k, "best_score":min(best_sample_score,x_bar_k_score,x_k_score), "charge":x_bar_k["c_bl"],"decharge":x_bar_k["d_bl"],"soc":x_bar_k["s_bl"],"gap_primal_dual":gap_primal_dual,"time":time.time()-start_time}, ignore_index=True)
        ...
        #for k in range(n_pre, K):
            ...
            delta_k = delta_optimize_pb_rd(x_k, x_bar_k, self.data, actual_time)
            #delta_k = 2/(100+k+2)
            print("=========iteration : ",k,"===========")
            ...
            for i in range(self.N):
                ...
                x_bar_k["z_1"][i]=(1-delta_k)*x_bar_k["z_1"][i]+delta_k*x_k["z_1"][i]
                x_bar_k["z_2"][i]=(1-delta_k)*x_bar_k["z_2"][i]+delta_k*x_k["z_2"][i]
                # u, v are fixed
    ...
    def save_data(self, df, n_pre, n_iteration):
        ...
        filename="../result/pb_reduced/" + "/"+instance_name+"/" + "PR_CFW2_"+ str(n_pre) + "_" + str(n_iteration) + "_" + time.strftime("%Y%m%d-%H%M%S") + ".json"
