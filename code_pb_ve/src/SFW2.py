...
class Frank_Wolfe_stochastique_gloutonne:
    ...
    def solve(self, actual_time, verbose=False, analyse=False, K=100, nk=50, n_pre=20, gap_calculate=False, optimize=True, nb_spb=1, active_criterion = "iterations", max_time=3600):
        """!!! gap primal-duale"""
        data=self.data
        # Initialisation
        df=0
        ...
        for k in range(K):
            ...
            if k==0:
                for i in range(self.N):
                    c_bl_i, d_bl_i, c_up_i, d_up_i ,u_bl_i, u_up_i, v_bl_i, v_up_i, s_bl_i, s_up_i, y = resolution_subpb(data, lambda_k,i,x_bar_k, verbose=verbose)
                    x_bar_k["c_bl"][i]=c_bl_i
                    x_bar_k["d_bl"][i]=d_bl_i
                    x_bar_k["c_up"][i]=c_up_i
                    x_bar_k["d_up"][i]=d_up_i
                    x_bar_k["s_bl"][i]=s_bl_i
                    x_bar_k["s_up"][i]=s_up_i
                    x_bar_k["u_bl"][i]=u_bl_i
                    x_bar_k["u_up"][i]=u_up_i
                    x_bar_k["v_bl"][i]=v_bl_i
                    x_bar_k["v_up"][i]=v_up_i
                    if optimize==True:
                        x_bar_k=self.solution_optimize(x_bar_k)
                #x_bar_k=deepcopy(x_g)
                x_k=deepcopy(x_g)
                update_solution=True
                #print(cost_g)
            else:
            ...
            #print("best_sample_score is : ",best_sample_score)
            print("x_bar_k_score is : ",x_bar_k_score)
            print("x_k_score is : ",x_k_score)

            print("Objective value is : ",min(x_bar_k_score,x_k_score))
            # Mettre a jour x_bar_k en fonction des scores de x_bar_k, x_k et du meilleur echantillon
            print(np.dot(lambda_k,np.sum(x_bar_k["c_bl"] - x_bar_k["d_bl"] - x_bar_k["c_up"] \
                                                        + x_bar_k["d_up"],axis=0)/self.N\
                            -np.sum(x_k["c_bl"] - x_k["d_bl"] - x_k["c_up"] + x_k["d_up"],axis=0)/self.N))
            print(self.function_h(x_bar_k)-self.function_h(x_k))
            ...
            if analyse:
                if k==0:
                    df = pd.DataFrame({"k": [k], "best_score":[min(x_bar_k_score,x_k_score)], "charge":[x_bar_k["c_bl"]],"decharge":[x_bar_k["d_bl"]],"charge_up":[x_bar_k["c_up"]],"decharge_up":[x_bar_k["d_up"]],"soc":[x_bar_k["s_bl"]],"soc_up":[x_bar_k["s_up"]],"gap_primal_dual":[gap_primal_dual]})
                else:
                    df = df.append({"k": k, "best_score":min(x_bar_k_score,x_k_score), "charge":x_bar_k["c_bl"],"decharge":x_bar_k["d_bl"],"charge_up":x_bar_k["c_up"],"decharge_up":x_bar_k["d_up"],"soc":x_bar_k["s_bl"],"soc_up":x_bar_k["s_up"], "gap_primal_dual":gap_primal_dual}, ignore_index=True)

        progress_bar.close()
        self.save_data(df,K,optimize)
    ...
    def save_data(self, df, n_iteration,optimize):
        ...
        if optimize ==False:
            filename="../result/pb_original/" + "/"+instance_name+"/" + "SFW2_" + str(n_iteration) + "_" + time.strftime("%Y%m%d-%H%M%S") + ".json"
        else:
            filename="../result/pb_original&booster/" + "/"+instance_name+"/" + "SFW2_" + str(n_iteration) + "_" + time.strftime("%Y%m%d-%H%M%S") + ".json"
