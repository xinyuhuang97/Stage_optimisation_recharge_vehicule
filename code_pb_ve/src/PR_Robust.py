...
class Frank_Wolfe_Reduced_Robust:
    def __init__(self,my_instance):
        self.instance=my_instance
        self.algo_name="PR_Robust"
    ...
    def solve(self, actual_time, verbose=False, analyse=False, K=100, nk=50, n_pre=20, gap_calculate=False, optimize=True, active_criterion="iterations", max_time=5000):
        """!!! gap primal-duale"""
        ...
        cpt_no_improvement=0
        k=0
        
        while k<20 and cpt_no_improvement<5:
            print("=========iteration : ",k,"===========")
            ...
            cost_best=self.objective_function(x_bar_k)
            if verbose:
                #print("best_sample_score",best_sample_score)
                #print("x_bar_k_score",x_bar_k_score)
                print("x_k_score",cost_best)
                print("after",self.objective_function(x_bar_k))
                print("====================")
            if (k+1)%10==0 :
            ...
            if analyse:
                if k==0:
                    df = pd.DataFrame({"k": [k], "best_score":[cost_best], "charge":[x_bar_k["c_bl"]],"decharge":[x_bar_k["d_bl"]],"soc":[x_bar_k["s_bl"]],"gap_primal_dual":[gap_primal_dual], "time":[time.time()-start_time]})
                else:
                    df = df.append({"k": k, "best_score":cost_best, "charge":x_bar_k["c_bl"],"decharge":x_bar_k["d_bl"],"soc":x_bar_k["s_bl"],"gap_primal_dual":gap_primal_dual, "time":time.time()-start_time}, ignore_index=True)
            k+=1
        ...
        while True:
            ...
            if analyse:
                df = df.append({"time_elapsed": time_elapsed,"k": k, "best_score":score, "charge":x_bar_k["c_bl"],"decharge":x_bar_k["d_bl"],"soc":x_bar_k["s_bl"],"gap_primal_dual":gap_primal_dual,"time":time.time()-start_time}, ignore_index=True)#, "gap_primal_dual":gap_primal_dual
            k+=1
        progress_bar.close()
    ...
    def save_data(self, df, n_iteration):
        ...
        filename="../result/pb_reduced/" + "/"+instance_name+"/" + "PR_Robust_" + str(n_iteration) + "_" + time.strftime("%Y%m%d-%H%M%S") + ".json"
